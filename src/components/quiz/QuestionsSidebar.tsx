...
//       });
//     }
//   }, [currentQuestion, currentQuestionIndex, currentQuestions]);
...
interface QuestionsSidebarProps {
  ...
  onQuestionClick: (index: number) => void;
  currentQuestions?: Question[]; // Only need this prop now
}
...
const QuestionsSidebar = ({
  ...
  answeredQuestions,
  onQuestionClick,
  currentQuestions = [], // Default empty array
}: QuestionsSidebarProps) => {
  // Memoized flagged questions map
            ...
            return (
              <button
                ...
                className={cn(
                  "aspect-square flex items-center justify-center rounded text-sm font-medium transition-all hover:opacity-80",
                  currentQuestionIndex === index && "ring-2 ring-primary",
                  answered?.isCorrect && "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300",
                  answered?.isCorrect === false && "bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300",
                  !answered && !isFlagged && "bg-gray-100 text-gray-700 dark:bg-gray-700/50 dark:text-gray-300",
                  !answered && isFlagged && "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-300"
                )}
              >
                <div className="flex flex-col items-center gap-0.5">
                  <span className="text-xs">{index + 1}</span>
